...
public class Z extends Tetrimino{
	...
	public boolean moverAbajo() {
		...
		boolean movimientoPosible = (bajarposible_0_270 || bajarposible_90_180); //si está en la última fila no podrá mover hacia abajo.
		if(movimientoPosible) {
			...
			switch(rotacion) {
				...
				case(90): {
					movimientoPosible = ( newP1.estaLibre() && newPosCentral.estaLibre() );
					if(movimientoPosible) {
				...
				case(270): {
					movimientoPosible = ( newP3.estaLibre() && newP2.estaLibre() );
					if(movimientoPosible) {
	...
	public void moverIzquierda() {
		...
		if(movizqposible_0_90 || movizqposible_180_270) {
			...
			switch(rotacion) {
				...
				case(90): {
					mover = newP2.estaLibre() && newP1.estaLibre() && newP3.estaLibre();
					if(mover) {
						ocuparDesocuparCuatroBloquesEnOrden(newP2, pos2, newP1, pos1, newP3, pos3, newPosCentral, poscentral);
					} 
					break;
				...
				case(270): {
					mover = newPosCentral.estaLibre() && newP3.estaLibre() && newP1.estaLibre();
					if(mover) {
