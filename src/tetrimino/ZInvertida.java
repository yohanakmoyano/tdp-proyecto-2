...
public class ZInvertida extends Tetrimino {
	...
	public boolean moverAbajo() {
		...
		if(movimientoPosible) {
			...
			switch(rotacion) {
				case(0): {
					movimientoPosible = ( newPosCentral.estaLibre() && newP3.estaLibre() && newP1.estaLibre());
					if (movimientoPosible) {
				...
				case(180): {
					movimientoPosible = ( newP3.estaLibre() && newP2.estaLibre() && newP1.estaLibre() );
					if (movimientoPosible) {
				...
				case(270): {
					movimientoPosible = ( newP3.estaLibre() && newPosCentral.estaLibre() );
					if (movimientoPosible) {
	...
	public void moverIzquierda() {
		...
		if(movizqposible_0_270 || movizqposible_90_180) {
			...
			switch(rotacion) {
				...
				case(90): {
					mover = newP3.estaLibre() && newP2.estaLibre() && newP1.estaLibre();
					if (mover) {
						ocuparDesocuparCuatroBloquesEnOrden(newP3, pos3, newP2, pos2, newPosCentral, poscentral, newP1, pos1);
					} 
					break;
	...
	public void moverDerecha() {
		...
		if(movderposible_0_270 || movderposible_90_180) {														// mover los bloques.
			Bloque newP1 = miGrilla.getBloque(pos1.getFila(), pos1.getColumna() + 1);
			Bloque newP2 = miGrilla.getBloque(pos2.getFila(), pos2.getColumna() + 1);
			Bloque newP3 = miGrilla.getBloque(pos3.getFila(), pos3.getColumna() + 1);
			Bloque newPosCentral = miGrilla.getBloque(poscentral.getFila(), poscentral.getColumna() + 1);
			...
			switch(rotacion) {
				...
				case(90): {
					mover = newPosCentral.estaLibre() && newP1.estaLibre() && newP3.estaLibre();
					if (mover) { 
						ocuparDesocuparCuatroBloquesEnOrden(newPosCentral, poscentral, newP1, pos1, newP3, pos3, newP2, pos2);
					}
					break;
				...
				case(270): {
					mover = newP2.estaLibre() && newP3.estaLibre() && newP1.estaLibre();
					if (mover) { 
