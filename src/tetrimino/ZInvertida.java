...
public class ZInvertida extends Tetrimino{
	...
	public boolean moverAbajo() {
		...
		if(movimientoPosible) {
			...
			if(movimientoPosible) {
				//Cuidado con el orden en el que se reemplazan/ocupan los bloques, debemos mover primero los dos de abajo (p1 y pc) y luego los otros dos de arriba (p2 y p3).
	...
	public void moverIzquierda() {
		if(pos1.getColumna() != 0) { //Si no me encuentro en la primer columna.
			...
			if(newP1.estaLibre() && newPosCentral.estaLibre()) {
				//Respetando que al mover este tetrimino, debemos tener cuidado con que antes de mover el bloque p3 debemos mover el bloque p2 y antes del bloque pc debo mover el bloque p1.
				newP1.ocupar(pos1.getBloqueGrafico());
				pos1.desocupar();
				pos1 = newP1;
